...
- A local DeviceChain cluster (`deploy/local/`), installed and bootstrapped. **`dcctl install local`
  ...
  **`ssl://localhost:1883`**.
  ...
  # want: subject=... CN = dc-nats.dci-<instance-id>   and a negotiated cipher
  ```

- **The broker's CA**, so the player can pin it. The bring-up mints a private CA; extract it once:

  ```bash
  kubectl -n dci-<instance-id> get secret dc-nats-tls -o jsonpath='{.data.ca\.crt}' | base64 -d > nats-ca.pem
  ```

  > 🔑 **Use `localhost`, never `127.0.0.1`.** Measured against a live bring-up: the broker leaf
  > carries `DNS:dc-nats`, `DNS:dc-nats.dci-<instance-id>`, `DNS:dc-nats.dci-<instance-id>.svc`,
  > `DNS:dc-nats.dci-<instance-id>.svc.cluster.local`, `DNS:localhost` — and **no IP SAN at all.** An IP
...
kubectl -n dci-<instance-id> get secret dc-nats-tls -o jsonpath='{.data.ca\.crt}' | base64 -d > /tmp/ca.pem
