...
| `checkpointIntervalSeconds` | 10 | Maximum time between checkpoints, so a quiet stream still commits. **At most 30**: a checkpoint is what acknowledges the stream, so an interval longer than half the broker's acknowledgement window makes every message on a quiet stream redeliver, and the service refuses it at startup. |
