...
- **Asset (Activo)** — la cosa del mundo real que un dispositivo monitorea, clasificada por un **tipo de activo** definido por el inquilino. Un tipo de activo puede publicar un **esquema de propiedades** versionado contra el que se validan las propiedades de sus activos, y los activos forman una **jerarquía padre/hijo** (un solo padre por activo, sin ciclos).
- **Area (Área)** — una ubicación espacial/organizacional con nombre, clasificada por un **tipo de área**. Los límites espaciales se modelan por separado como [geocercas](./geofencing.md), no como campos del área.
- **Customer (Cliente)** — un propietario organizacional, clasificado por un **tipo de cliente**.
- **Groups (Grupos)** — un **grupo de entidades** uniforme que agrupa miembros de **una sola familia** — dispositivos, activos, áreas o clientes — elegida al crear el grupo. La membresía es **estática** (una lista explícita de miembros) o **dinámica** — un selector guardado sobre los atributos de los miembros, resuelto en el momento de la lectura (ver [Facetas y grupos dinámicos](#facets-and-dynamic-groups)).

Los dispositivos, activos, áreas, clientes y grupos se direccionan de manera uniforme mediante un **tipo de entidad + token estable por inquilino**, lo que permite que las relaciones, los grupos y la indexación de eventos operen de forma genérica sobre ellos. (Los tipos de dispositivo y los perfiles de dispositivo son configuración que un dispositivo resuelve a través de ellos, no entidades direccionables en este sentido.)
...
Un perfil de dispositivo declara los **comandos** que aceptan sus dispositivos, y un
comando emitido se persiste y se rastrea a través de un ciclo de vida en lugar de
enviarse y olvidarse. Ambos se describen en [Comandos](./commands.md).
