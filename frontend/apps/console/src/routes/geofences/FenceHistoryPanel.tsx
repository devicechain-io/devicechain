...
export function FenceHistoryPanel({ entity }: { entity: GeoFence }) {
  ...
  if (current.error && !current.data) {
    return <ErrorBanner message={errMessage(current.error)} />;
  }

  // 🔴 Zero is not "version zero". It means no fence set has ever been minted,
  // which for a tenant that has just created its first fence is a transient the
  // operator should not have to decode from a number.
  if (latest === 0) {
    return <p className="text-muted-foreground text-sm">{t('entities:geofenceHistoryNone')}</p>;
  }

  const found = snapshot.data
    ? lookupInSnapshot(snapshot.data.fences, entity.token)
    : null;
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => setVersion(Math.max(1, chosen - 1))}
            disabled={chosen <= 1}
          >
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => setVersion(Math.min(latest, chosen + 1))}
      ...
      {snapshot.error && !snapshot.data && <ErrorBanner message={errMessage(snapshot.error)} />}
