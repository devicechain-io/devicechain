...
check() {
  ...
  # :/ anchors the pathspec at the repository root: a relative pathspec that matches
  # nothing makes `git diff --quiet` exit 0 — clean — whatever the file holds. The
  # comparison is against the INDEX, so a hash someone has staged counts as committed.
  local rc=0
  git -C "$dir" diff --quiet -- :/go.work.sum || rc=$?
  case "$rc" in
    0)
    ...
    1) ;;
    *)
      echo "check-go-work-sum: git diff failed (exit $rc); refusing rather than reporting clean" >&2
      return 2 ;;
  esac
  ...
  # that was already there. 🔴 LC_ALL=C on comm as well as on sort: comm checks its
  # input's order in its OWN locale, and a real go.work.sum has upper-case module paths
  # that sort differently under en_US — comm then rejects the input as unsorted.
  local staged added
  staged="$(mktemp)" || return 2
  if ! git -C "$dir" show :go.work.sum >"$staged"; then
    rm -f "$staged"
    echo "check-go-work-sum: could not read the staged go.work.sum; refusing rather than reporting clean" >&2
    return 2
  fi
  if ! added="$(LC_ALL=C comm -13 <(LC_ALL=C sort -u "$staged") <(LC_ALL=C sort -u "$dir/go.work.sum"))"; then
    rm -f "$staged"
  ...
  if [ -n "$added" ]; then
    ...
    git -C "$dir" --no-pager diff -- :/go.work.sum
  fi
  return 1
}
...
self_test() (
  ...
  echo "check-go-work-sum self-test passed: the script, run by path from a module directory, fails on unstaged hashes with and without --module and lists every one of them and nothing already committed; a staged hash passes and does not hide a later unstaged one; other dirty files are ignored; a removal fails and shows the change; a missing file or unknown argument is refused."
)
