...
// readAndHandleOne reads one resolved event and hands it to the persistence workers,
// reporting whether the read loop would have STOPPED — the per-message seam ProcessMessage
// used to provide before the loop itself moved into messaging.RunConsumer.
//
...
func readAndHandleOne(p *EventPersistenceProcessor, ctx context.Context) bool {
	msg, err := p.ResolvedEventsReader.ReadMessage(ctx)
	if err != nil {
		return true
	}
