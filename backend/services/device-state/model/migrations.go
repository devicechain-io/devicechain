...
// six-migration chain that preceded it existed only to walk developer databases forward one
// column at a time, and until v1.0.0 there is no released version to upgrade from — an
// existing instance is recreated, not migrated. The IDs of those removed migrations stay
// recorded in any database that ran them, which gormigrate tolerates because the RdbManager
// runs with ValidateUnknownMigrations off (backend/core/rdb). Don't enable that validation
// without first reconciling those orphaned rows.
//
...
// snapshot types precisely so it does not track the live models (see baseline_snapshot.go
