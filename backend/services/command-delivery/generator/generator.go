...
// GetConfigurationResources gets configuration CRs that should be created in tooling.
func (ResourceProvider) GetConfigurationResources() ([]gen.ConfigurationResource, error) {
	...
	name := "command-delivery-default"
	msconfig := config.NewCommandDeliveryConfiguration()
	content, err := gen.GenerateMicroserviceConfig(name, "command-delivery", "devicechain-io/command-delivery:0.0.1", msconfig)
	if err != nil {
		return nil, err
	}
