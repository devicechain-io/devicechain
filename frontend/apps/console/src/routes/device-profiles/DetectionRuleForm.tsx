...
export function ruleTypeOptions(t: TFunction): ComboboxOption[] {
  return RULE_TYPES.map((value) => ({
    value,
    label: t(`ruleType${ruleTypeKey(value)}Label`),
    description: t(`ruleType${ruleTypeKey(value)}Description`),
  }));
}
...
export function DetectionRuleForm({
  ...
  // The stored bytes this drawer opened on, if any: the rule being edited, else a handed-off
  // draft. Everything below that asks "what did we open?" reads this rather than re-deriving
  // which of the two it was.
  const source = editing ? entity.definition : (initialDefinition ?? null);
  const initial = useMemo(() => (source == null ? null : parseDefinition(source)), [source]);
  ...
  // the life of the drawer; recomputing it on every keystroke invited the reading that it
  // tracks what the operator is typing, which would be a different and wrong check.
  const lossyOpen = useMemo(
