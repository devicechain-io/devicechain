...
// stepClaimAndDeclare takes the cluster lock and records what this run is about
// to build, then reads it back as the input to everything downstream.
//
...
func stepClaimAndDeclare(ctx context.Context, st *State) error {
	...
	if st.DryRun {
		...
		doing("claiming the cluster and declaring the instance")
		fmt.Println()
		wouldDo(fmt.Sprintf("take the cluster lock and declare instance %q (provider %s, profile %s)",
			st.Instance, spec.Provider, spec.Profile))
		if err := reportExistingClaim(ctx, st); err != nil {
			return err
		}
		return nil
	}
	...
	if st.OperatorNamespace == "" {
		// Fail loudly rather than defaulting. This value comes from the rendered
		// overlay one step earlier; an empty one means that step did not run, and
		// guessing a namespace here would take the lock somewhere nothing else
		// looks — a lock that protects nothing while appearing to work.
		return fmt.Errorf("the operator namespace is not known, so there is nowhere to take the cluster lock; " +
			"this step must run after the core components step")
	}

	doing("claiming the cluster")
	claim, err := AcquireClaim(ctx, typed, st.OperatorNamespace, st.Instance)
	if err != nil {
		return err
	}
	st.Claim = claim
...
func reportExistingClaim(ctx context.Context, st *State) error {
	...
	fmt.Println(color.YellowString("  %s", heldError(lease).Error()))
	return nil
}
