...
CREATE INDEX idx_commands_queued_device ON "command-delivery".commands USING btree (tenant_id, device_token, id) WHERE ((deleted_at IS NULL) AND ((status)::text = 'QUEUED'::text));
