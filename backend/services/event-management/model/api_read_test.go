...
func seedEvent(t *testing.T, api *Api, tenant string, deviceToken string, occurred time.Time) {
	t.Helper()
...
func TestEvents_TenantScopedRead(t *testing.T) {
	...
	seedEvent(t, api, "A", "device-100", base.Add(1*time.Minute))
	seedEvent(t, api, "A", "device-100", base.Add(2*time.Minute))
	// Tenant A: one row for a different device, outside the device filter.
	seedEvent(t, api, "A", "device-999", base.Add(1*time.Minute))
	// Tenant B: a matching device/time row that must NOT leak into A's results.
	seedEvent(t, api, "B", "device-100", base.Add(1*time.Minute))
	...
	for _, ev := range results.Results {
		...
		if ev.DeviceToken != "device-100" {
			t.Fatalf("device filter leaked device %q", ev.DeviceToken)
		}
	}
...
func TestEvents_FailsClosedWithoutTenant(t *testing.T) {
	...
	seedEvent(t, api, "A", "device-100", time.Now())
