...
| **operator** | Un operador basado en controller-runtime que es dueño del recurso personalizado `Instance` y de su ciclo de vida. Hoy su bucle de reconciliación solo observa el recurso; la agregación del estado de disponibilidad de los Deployments renderizados y la recarga en caliente de configuración son trabajos pendientes planificados que se incorporarán a ese bucle. Las cargas de trabajo en sí son renderizadas por el chart de Helm; los inquilinos son registros de base de datos del plano de control, no recursos reconciliados. |
