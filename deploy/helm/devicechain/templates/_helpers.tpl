...
{{/*
devicechain.egressDeniedIPv4 / devicechain.egressDeniedIPv6 — the address space a
tenant destination may not reach, rendered as the `except` entries of the
NetworkPolicy's two wide ipBlocks.

WHY THESE ARE TEMPLATES AND NOT VALUES, WHICH IS THE WHOLE POINT.

They used to be `networkPolicy.blockedIPv4Ranges` / `blockedIPv6Ranges` in
values.yaml, and that made the deny list operator-supplied data. Helm REPLACES a
list wholesale rather than merging it, so `--set networkPolicy.blockedIPv4Ranges={1.2.3.4/32}`
— or the same thing in a values file, which is the path an operator is told to use —
rendered a policy whose only exception was that one address. It passed every guard,
it was accepted by the API server, and it permitted all of 10.0.0.0/8 and the
instance metadata service while still looking exactly like a control. That is the
failure this object exists to prevent, reachable by one plausible edit.

This table is invariant per release. It is gated against the Go guard's deny table
by hack/check-egress-ranges.sh precisely BECAUSE both halves must name the same
address space, so it is not a thing a deployment gets to redefine — an operator who
changes one half has silently created two different boundaries wearing one name.
Widening for a specific destination is what networkPolicy.additionalAllowedCidrs is
for, and narrowing further is a change to both halves, in a commit, with the gate
to prove they still agree.

🔴 KEEP THESE IN STEP WITH backend/core/egress/ranges.go. The gate compares the
RENDERED policy against that table as the compiled package reports it, so a change
to either side fails CI until the other follows. One prefix is deliberately absent
here — the IPv4-mapped range — because Kubernetes rejects a mapped address inside an
ipBlock and the whole policy is then invalid. The gate derives that exception from
the address itself rather than from a hard-coded list, so nothing here has to
declare it.
*/}}
{{- define "devicechain.egressDeniedIPv4" -}}
- "0.0.0.0/8"
- "10.0.0.0/8"
- "100.64.0.0/10"
- "127.0.0.0/8"
...
{{- define "devicechain.egressDeniedIPv6" -}}
- "::1/128"
- "::/128"
- "64:ff9b:1::/48"
- "100::/64"
