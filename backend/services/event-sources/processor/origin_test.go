...
// Through the real gate: an untrusted origin is metered in the bounded pool, and the
// ZERO Origin — a transport that forgot to say — is treated the same way, never as
// authenticated.
func TestUntrustedAndUnsaidOriginsAreBounded(t *testing.T) {
	for _, origin := range []Origin{OriginUntrusted, 0} {
		...
		gate := NewRateGate(live, backlog, nil)

		for i := 0; i < 5000; i++ {
			gate("http", fmt.Sprintf("invented-%d", i), time.Time{}, false, origin)
		}
		confirmed, pooled, overflow := live.BucketCounts()
		require.Equalf(t, 0, confirmed, "origin %d: no invented name may get a confirmed bucket", origin)
		assert.LessOrEqualf(t, pooled, 1024, "origin %d: the pool is bounded", origin)
		assert.Truef(t, overflow, "origin %d: names past the pool share the overflow", origin)
		c, p, o := backlog.BucketCounts()
		assert.Truef(t, c == 0 && p == 0 && !o, "origin %d: an untrusted message never reaches the backlog limiter", origin)
	}
}
