...
// NewBaselineSchema materializes the whole command-delivery schema in one step: the
// persisted, lifecycle-tracked command (ADR-012 #4) and its per-tenant token index.
//
// This REPLACES the former three-migration chain, collapsed pre-GA. Until v1.0.0 there is
// no released version to upgrade FROM, so that chain only ever walked a maintainer's
// laptop forward; the history has no value before there is something to migrate from. Its
// equivalence to this migration is proven by hack/migration-diff.sh verify against the
// golden captured from the chain.
//
// 🔴 TWO OF THOSE THREE MIGRATIONS ARE NOT REPRESENTED BY DDL HERE, for opposite reasons,
// and both are deliberate:
//
//   - the drop of commands.delivered_time folded in as an ABSENCE — the column is simply
//     not in the snapshot type. See baseline_snapshot.go.
//   - the default-TTL migration was a BACKFILL (`UPDATE ... SET expires_at = now() + ...
//     WHERE expires_at IS NULL`) and is DROPPED, not folded. It stamped a horizon onto rows
//     that already existed in a maintainer's database; a baseline runs against an empty
//     schema where there is nothing to backfill, and carrying it forward would only run an
//     UPDATE over zero rows. New commands get their TTL from CreateCommand, which is where
//     that behaviour lives going forward.
//
// The distinction matters because migration-diff CANNOT see it: pg_dump --schema-only
// captures no rows, so dropping a genuine SEED here would pass just as quietly. This area
// has no seed — nothing in it is unusable without a row — which is why nothing is folded in
// to replace the backfill.
//
...
// not "update" the snapshot type it builds from. Anything appended must be individually
...
func NewBaselineSchema() *gormigrate.Migration {
	return &gormigrate.Migration{
		...
		Migrate: func(tx *gorm.DB) error {
			// The SNAPSHOT type in baseline_snapshot.go, never the live model.
			if err := tx.AutoMigrate(&command{}); err != nil {
				return err
			}
			// ADR-042 P1: a token is unique within a tenant among LIVE rows only. Not
			// declared as a struct tag because gorm cannot express a partial index, and a
			// plain UNIQUE(token) would both collide across tenants and keep a
			// soft-deleted row's token locked forever.
			return createTenantTokenIndex(tx, &command{})
		},
...
// old one, from a PR that never touched command-delivery. That is the same hazard the
// inlined mixins in baseline_snapshot.go exist to remove, one level up, and the reason the
// snapshot rule outranks DRY inside a migration. Do not "clean this up".
//
...
func createTenantTokenIndex(tx *gorm.DB, model any) error {
	...
	// already scoped by the schema it lives in, and the golden it must reproduce was built
	// from the unprefixed form.
	bare := stmt.Table
