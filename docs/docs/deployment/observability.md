...
- It only sees a stream the pod has read at least once. A pod that has never been able to read a
  stream since it started, including one restarted after the problem began, does not fire it.
