...
func TestDispatchRaiseAlarmEnabled(t *testing.T) {
	...
	d2 := NewDispatcher(fakeResolver{rule: raiseAlarmRule("over-temp"), found: true}, nil, alarms2, nil, nil, newFakeMetrics())
...
func TestDispatchRaiseAlarmCarriesValue(t *testing.T) {
	...
	ev.Value = &v
	alarms := &fakeAlarmSink{}
	d := NewDispatcher(fakeResolver{rule: raiseAlarmRule(""), found: true}, nil, alarms, nil, nil, newFakeMetrics())
	...
	alarms2 := &fakeAlarmSink{}
	d2 := NewDispatcher(fakeResolver{rule: raiseAlarmRule(""), found: true}, nil, alarms2, nil, nil, newFakeMetrics())
	d2.Dispatch(context.Background(), evt())
