...
| **operator** | A controller-runtime operator that owns the `Instance` custom resource and its lifecycle. Today its reconcile loop only observes the resource; readiness-status aggregation across the rendered Deployments and config hot-reload are planned follow-ups that land on that loop. Workloads themselves are rendered by the Helm chart; tenants are control-plane database records, not reconciled resources. |
