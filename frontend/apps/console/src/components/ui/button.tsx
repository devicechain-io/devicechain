...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading, children, disabled, ...props }, ref) => {
    ...
    // When asChild, Slot requires exactly one child element.
    // Extra expressions (even falsy ones) create an array in props.children,
    // causing React.Children.only to throw.
    if (asChild) {
      return (
        <Comp
    ...
    return (
      ...
      >
        {loading && (
          <Loader2 className="animate-spin -ml-0.5 size-4" />
        )}
        {children}
      </Comp>
    )
