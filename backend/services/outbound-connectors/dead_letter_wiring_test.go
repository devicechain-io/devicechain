...
// hands it to the package, and newDeadLetterIndexWriter builds the writer.
...
func TestADeadLetterLandsOnThePlatformStreamWithThisServiceAsSource(t *testing.T) {
	...
	Microservice = &core.Microservice{InstanceId: instance, FunctionalArea: "outbound-connectors"}
	...
	nmgr := messaging.NewNatsManager(Microservice, core.NewNoOpLifecycleCallbacks(),
		func(m *messaging.NatsManager) error {
			var err error
			var w messaging.MessageWriter
			if w, err = newDeadLetterIndexWriter(m); err == nil {
				// The dispatch consumer builds its index sink from this writer and the
				// service's producer; building it the same way here is the processor's
				// choice, pinned in the processor's own tests.
				sink = DeadLetters.NewIndexSink(w, func(error) {})
			}
			return err
		})
	...
	require.Equal(t, "outbound-connectors", letter.Source, "the letter does not name this service as its source")
	require.Equal(t, deadletter.KindConnectorDispatch, letter.Kind)
