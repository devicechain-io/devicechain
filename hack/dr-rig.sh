...
backup_creds_file="$work/backup-credentials.json"
...
# moment those credentials became a Secret rather than a variable. The rig
# exercising the supported path is worth more than the rig having its own.
write_credentials() {
  [[ -s "$minio_env_file" && -s "$backup_creds_file" ]] && return 0
...
EOF
  cat >"$backup_creds_file" <<EOF
{
  "endpointUrl": "http://$minio_container:9000",
  "bucketRdb": "$bucket_rdb",
  "bucketTsdb": "$bucket_tsdb",
  "accessKeyId": "$user",
...
# cannot open.
backup_args() {
  [[ -s "$backup_creds_file" ]] || fail "no object-store credentials at $backup_creds_file.
...
because the bucket it has to read was written under those exact credentials."
  printf '%s' "--backup-credentials-file=$backup_creds_file"
}
