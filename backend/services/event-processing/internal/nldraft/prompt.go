...
A "when.cel" MUST NOT test a geofence and a measurement together. A leaf that calls geo.inFence() and also reads m is REJECTED at compile — not narrowed, rejected — because a location event carries no measurements and a measurement event carries no position, so no event could ever satisfy both and the rule would never fire. For "alarm when a truck in the restricted zone is over 5 degrees", do NOT emit geo.inFence("restricted") && m["tempC"] > 5. Emit the containment test alone, or express the second condition as a device ATTRIBUTE, which a location event does carry: geo.inFence("restricted") && attr["tempC"] > 5.
