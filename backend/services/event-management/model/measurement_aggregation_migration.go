...
// (BucketedMeasurements): it filters by tenant_id (+ usually device_id and a
...
// per-metric) bucketed query index-only over its slice.
	...
	return &gormigrate.Migration{
		...
		Migrate: func(tx *gorm.DB) error {
			return tx.Exec(`CREATE INDEX IF NOT EXISTS idx_measurement_tenant_device_name_time ` +
				`ON "event-management"."measurement_events" (tenant_id, device_id, name, occurred_time DESC);`).Error
		},
