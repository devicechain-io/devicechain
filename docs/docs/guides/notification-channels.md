...
Note the severity here is **uppercase**, because it is the *alarm's* severity. A detection rule's own authoring severity is lowercase (`major`), and it is uppercased when the alarm is raised — so a notification rule always matches the uppercase form. A rule whose severity is not one of those values (a lowercase `major`, say) is **refused at write**, rather than stored as a rule that could never match.
