...
- **An idempotency key on every dispatch.** Each dispatch carries a content-addressed **idempotency key** derived from the firing: the same detection and action produce the same key on every redelivery and every replay. Delivery is at-least-once, and DeviceChain does not deduplicate dispatches itself, so a dispatch delivered twice is sent twice. The key is forwarded so the receiver can recognize and drop the duplicate: as the `X-DC-Idempotency-Key` header on a webhook, as an `idempotency_key` record header on Kafka, and as an `idempotency_key` message attribute on SNS and SQS. An MQTT publish carries no key, because MQTT 3.1.1 has nowhere to put it.
