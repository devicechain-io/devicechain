...
// 🔴 THE CASES BELOW ARE THE THROW SITES IN packages/client/src/transport.ts, READ OUT
// OF IT RATHER THAN IMAGINED. That is the whole strength of this file: a predicate over
...
const REJECTED = 'signIn:invalidCredentials';
...
describe('serverRejectedRequest', () => {
  ...
  // 🔴 EVERY ONE OF THESE IS A CASE THE TWO OBVIOUS PREDICATES GET WRONG, and each one
  // is a viewer being told to re-type a password that was never the problem.
  it.each([
    // `instanceof` alone says true here: fetch() failing throws the same class.
    ['a dead network (fetch rejected)', new GraphQLRequestError('Failed to fetch', 0)],
    // `status !== 0` says true here: a 503 has a real status and no errors array.
...
describe('signInErrorKey', () => {
  it('returns the caller’s rejection key when the server said no', () => {
    ...
    expect(signInErrorKey(err, 'signIn:enterTenantFailed')).toBe('signIn:enterTenantFailed');
  });
...
// well against keys no catalog defines — i18next answers a missing key with the key's
...
describe('every sign-in failure has real text in every shipped locale', () => {
  ...
  it('covers exactly the keys signInErrorKey can return', () => {
    const rejected = ['signIn:invalidCredentials', 'signIn:enterTenantFailed'];
