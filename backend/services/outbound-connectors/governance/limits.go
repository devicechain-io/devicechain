...
// Package governance resolves per-tenant OUTBOUND limits (ADR-060 SD-3) for the
// egress rate limiter: it fetches a tenant's outbound overrides from
// user-management and caches them in memory, refreshing out of band so the hot
// dispatch path never blocks on a network call, and fails open to the platform
// default so a slow or unreachable authority degrades to metered-at-default, never
// to unmetered.
//
// This deliberately mirrors event-sources' ingest governance resolver rather than
// sharing it: ingest and outbound are INDEPENDENT governance dimensions (a tenant
// may ingest heavily yet fan out few outbound calls, or the reverse), read from
// distinct fields of the tenantGovernance query, and may diverge later (e.g.
// per-connector weighting). Promoting the resolver mechanics to core is a possible
// future consolidation, deferred here to avoid refactoring the shipped, live ingest
// hot path for a defense-in-depth egress limiter.
package governance
