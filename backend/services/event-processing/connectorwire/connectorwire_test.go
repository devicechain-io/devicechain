...
func TestConnectorDispatchRoundTrip(t *testing.T) {
	...
	triggered := time.Date(2026, 7, 13, 12, 0, 3, 250_000_000, time.UTC)
	cases := []*ConnectorDispatchRequest{
		{
			Kind: ConnectorKindHTTPCall, Tenant: "acme", DeviceToken: "device-1", RuleID: "acme/p@1/r1",
			Edge: "raised", OccurredTime: occurred, TriggeredAt: triggered, IdempotencyKey: "tok1", Payload: `{"a":1}`,
		...
		},
		{
			...
			Edge: "raised", OccurredTime: occurred, TriggeredAt: triggered, IdempotencyKey: "tok2",
	...
	for _, in := range cases {
		...
		if out.Kind != in.Kind || out.Tenant != in.Tenant || out.IdempotencyKey != in.IdempotencyKey ||
			...
			!out.TriggeredAt.Equal(in.TriggeredAt) {
