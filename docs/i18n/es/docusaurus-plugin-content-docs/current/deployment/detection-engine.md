...
| `checkpointIntervalSeconds` | 10 | Tiempo máximo entre puntos de control, para que un flujo tranquilo también confirme. **Como máximo 30**: un punto de control es lo que confirma el flujo, así que un intervalo mayor que la mitad de la ventana de confirmación del broker hace que cada mensaje de un flujo tranquilo se vuelva a entregar, y el servicio lo rechaza al arrancar. |
