...
locals {
  ...
  # trips the guard.
  #
  # 🔑 ONE ENTRY, WHERE THE PRE-SPLIT ROOT HAD TWO. The event store's entry moved
  # to the instance root with the store itself. The map shape is kept rather than
  # flattened to a single object because the reason it was written per-store still
  # holds: adding a store here is one map entry, and an absent entry is visible in
  # a way a missing copy of a 40-line block is not.
...
resource "terraform_data" "restore_guard" {
  count = local.rdb_restore == null ? 0 : 1

  input = var.restore_rdb_from
  ...
  lifecycle {
    ...
    precondition {
      condition     = var.backup_server_name_rdb != "" && var.backup_server_name_rdb != var.restore_rdb_from
      error_message = <<-EOT
...
# 🔴 THE TWO STORES MUST NOT SHARE A BUCKET, and after the split this root is the
# only place that can still say so.
#
# It creates BOTH buckets — the relational store's, and the event store's, which
# an instance root archives into without being able to see this one. Their
# serverNames are the only thing that would keep two archives apart inside one
# bucket, and a shared bucket also collapses two independent restore domains into
# one retention policy.
#
# 🔑 It is a check rather than a derivation because the names are supplied: dcctl
# writes both, and hack/dr-rig.sh sets them by hand. dcctl checks the same thing
# in backupcreds.go, and the doubling is deliberate for the usual reason — that
# one fires before a bootstrap starts, this one fires for whoever runs tofu
# directly.
resource "terraform_data" "bucket_separation_guard" {
  count = local.backups_on ? 1 : 0

  input = "${var.backup_bucket_rdb}|${var.backup_bucket_tsdb}"

  lifecycle {
    precondition {
      condition     = var.backup_bucket_rdb != var.backup_bucket_tsdb
      error_message = <<-EOT
        backup_bucket_rdb and backup_bucket_tsdb are the same bucket.
