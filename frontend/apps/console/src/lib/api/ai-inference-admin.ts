...
// endpoint, model, params, write-only API key} config; at most one is the ACTIVE
// provider used for NL→rule authoring ("default of none" when none is active). The
// API key is write-only: it goes in on create/update (`secret`) and never comes back
// out — the read side exposes only `hasSecret`.
//
...
export async function getActiveAiProvider(): Promise<ActiveAiProvider | null> {
  const data = await gql('ai-inference/admin', ACTIVE_AI_PROVIDER);
...
export async function setActiveAiProvider(
  ...
  const data = await gql('ai-inference/admin', SET_ACTIVE_AI_PROVIDER, { token });
...
export async function clearActiveAiProvider(): Promise<boolean> {
  const data = await gql('ai-inference/admin', CLEAR_ACTIVE_AI_PROVIDER);
