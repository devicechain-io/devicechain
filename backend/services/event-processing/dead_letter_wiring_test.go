...
func TestADeadLetterLandsOnThePlatformStreamWithThisServiceAsSource(t *testing.T) {
	...
	require.Equal(t, "event-processing", letter.Source, "the letter does not name this service as its source")
	require.Equal(t, deadletter.KindDetectionAction, letter.Kind)
