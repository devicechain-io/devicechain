...
func startShutdownFixture(t *testing.T) (*shutdownProbe, string) {
	...
	NotificationProcessor = processor.NewNotificationProcessor(Microservice, nil,
		core.NewNoOpLifecycleCallbacks(), nil, nil, processor.NewNotifyMetrics(Microservice))
	require.NoError(t, NotificationProcessor.Initialize(context.Background()))

	// Both of these are nil-guarded in the stopper; there is nothing to stop.
	RetentionSweeper, EscalationScheduler = nil, nil
...
// has no such reason: the alarm consumer is the only thing of its own that touches the
// broker, and it is stopped above both of the lines under test.
