...
func TestTheFenceCeilingMaximumIsStatedInBytesSomewhereReal(t *testing.T) {
	...
	// Read from the config package that owns it rather than restated. Its twin in
	// device-management was a literal under a comment claiming this value "is deployment
	// configuration rather than a Go constant" — the deployed value is, the DEFAULT is not,
	// and a false sentence justifying a copy is how a number drifts away from its source.
	defaultBrokerCeiling := int(config.DefaultStreamMaxMsgSize)
...
// tenants resident at once) and the maximum is a HALF of it. Note what the half does NOT buy —
// MaxTenantGeometryPositions' own comment and the cache both say it bounds what a tenant may
// ADD, not what it HOLDS, so a churning tenant can still evict its neighbours; "no single tenant
// may evict every other" is the stronger claim this file used to make and must not make again.
// The cache constant itself lives in event-processing, which this package must not
...
func TestTheMaximumCapsAreDeliberatelyNotMutuallyReachable(t *testing.T) {
	if MaxGeoFenceCeiling*MaxGeoFencePositionCeiling <= MaxTenantGeometryPositions {
		t.Errorf("the maximum fence count (%d) times the maximum position ceiling (%d) is %d, which "+
			...
			"which, because the budget being the binding constraint is what bounds how much one "+
			"tenant may ADD to that cache", MaxGeoFenceCeiling, MaxGeoFencePositionCeiling,
