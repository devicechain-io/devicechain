...
| `command_overflow_blocked_total` | Times the adapter stopped reading commands because it could not set them aside fast enough. It rises only while command-delivery is slow or unreachable, and then every LwM2M command waits. |
