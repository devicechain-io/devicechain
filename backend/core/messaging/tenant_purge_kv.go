...
func KvPurgeExemptions() []string {
	return []string{
		...
		kv.BucketCredentialAttempts + ": sign-in backoff state, one entry per recently failing " +
			"principal, keyed by a hash of the presented email or OAuth client_id. Neither names " +
			"a tenant — a sign-in attempt belongs to the PERSON, like the refresh-token store — and " +
