...
// anchors, attr) hold. A resolved event carries a small, bounded set of measurements and
// anchors, and a device a small bounded set of attributes; without a hint cel-go treats a
// map as unbounded and every predicate that iterates or indexes one estimates to an infinite
// cost, which would reject even trivial rules. This is a *static estimation* bound only — it
// does not cap the runtime map; the per-tenant ingest limits (ADR-023) bound the actual event
// size, and the CostLimit set on the Program is the runtime backstop.
...
// EstimateSize bounds the map variables (and any node typed as one of them) to
