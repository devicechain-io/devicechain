...
func TestSubscriptionsFollowGroups(t *testing.T) {
	...
	all := NewClient(config.SparkplugConfiguration{HostId: "h"}, Broker{}, clk, nil, Metrics{})
	assert.Equal(t, []string{"spBv1.0/#"}, all.subscriptions())

	scoped := NewClient(config.SparkplugConfiguration{HostId: "h", Groups: []string{"plant-a", "plant-b"}}, Broker{}, clk, nil, Metrics{})
