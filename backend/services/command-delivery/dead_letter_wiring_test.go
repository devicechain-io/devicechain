...
func TestADeadLetterLandsOnThePlatformStreamWithThisServiceAsSource(t *testing.T) {
	...
	Microservice = &core.Microservice{InstanceId: instance, FunctionalArea: "command-delivery"}
	...
	require.Equal(t, "command-delivery", letter.Source, "the letter does not name this service as its source")
	require.Equal(t, deadletter.KindCommandResponse, letter.Kind)
