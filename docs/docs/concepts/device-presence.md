...
The presence *source* is not surfaced in any interface yet. The console's Connectivity tab and the [MCP](./mcp.md) device tools both show the resulting online/offline state and the last connect, disconnect, and activity times — but not whether that state was asserted by the transport or inferred from a timeout. It is readable programmatically, though: `presenceSource` is a field on `device-state`'s `DeviceState` type, returning `ASSERTED` or `INFERRED`, so an integration can tell the two apart even while no screen does.
