...
func (p *shutdownProbe) assertGraphQLStoppedFirst(t *testing.T) {
	...
	require.NoError(t, p.inFlightErr,
		"a subscriber whose socket was still open when shutdown began could not attach to the "+
			"tenant's live event feed: the NATS manager was stopped before the HTTP server drained, "+
			"so the stream the events subscription reads was already gone")
...
func startShutdownFixture(t *testing.T) (*shutdownProbe, string) {
	...
	prevPersist, prevRecon, prevSweep, prevSvc := EventPersistenceProcessor, EntityAnchorReconciler, AnchorSweep, Svc
	t.Cleanup(func() {
		Microservice, GraphQLManager, NatsManager, RdbManager = prevMs, prevGql, prevNats, prevRdb
		EventPersistenceProcessor, EntityAnchorReconciler, AnchorSweep, Svc = prevPersist, prevRecon, prevSweep, prevSvc
	})

	area := fmt.Sprintf("event-management-shutdown-%d", time.Now().UnixNano())
	...
	// The exact call the events subscription resolver makes, on the real connection.
	probe.inFlight = func() error {
		_, err := NatsManager.SubscribeLive(context.Background(), "acme", streams.ResolvedEvents)
		return err
	}
	...
	RdbManager = rdb.NewRdbManager(Microservice, core.NewNoOpLifecycleCallbacks(), nil,
		Microservice.InstanceConfiguration.Persistence.Tsdb, mscfg.MicroserviceDatastoreConfiguration{})
	...
	EventPersistenceProcessor = processor.NewEventPersistenceProcessor(Microservice, nil, nil,
		core.NewNoOpLifecycleCallbacks(), nil, processor.NewPersistMetrics(Microservice))
	require.NoError(t, EventPersistenceProcessor.Initialize(context.Background()))
	EntityAnchorReconciler = processor.NewEntityAnchorReconciler(Microservice, nil, nil,
		core.NewNoOpLifecycleCallbacks())
	require.NoError(t, EntityAnchorReconciler.Initialize(context.Background()))

	// The reconciliation sweep is nil-guarded in the stopper; there is nothing to stop.
	AnchorSweep = nil
...
// SUBSCRIPTION: the events subscription reads the tenant's resolved-event stream over
