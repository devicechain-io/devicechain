...
- device (string device token), occurred (timestamp), m (map of measurement key to number, e.g. m["tempC"]), attr (map of device-attribute key to number — a key is present only while the device has a NUMERIC value for it at SERVER or SHARED scope, so ALWAYS test presence ("k" in attr) before reading attr["k"]), anchors (map of anchor-type to token), geo (geofence containment — see below).

A threshold or duration "when.cel" that is true on every event from every device WITHOUT the attribute, whatever the event carries, is REJECTED at compile — e.g. !("tempLimit" in attr) || m["tempC"] > attr["tempLimit"]. For "per-device limit, else 80" emit the fallback as its own measurement test: "tempC" in m && ("tempLimit" in attr ? m["tempC"] > attr["tempLimit"] : m["tempC"] > 80.0).

Geofencing is written geo.inFence("<fence-token>"), which is true when the event's reported position is inside that fence. It is the ONLY geofence operation: geo is opaque, so a rule cannot read a latitude, a longitude, a fence list or a fence's geometry out of it, and no other geo.* function exists. Use it for "alert me when a vehicle leaves the yard" (!geo.inFence("yard")) or "when an asset enters the restricted area" (geo.inFence("restricted")).

A "when.cel" MUST NOT test a geofence and a measurement together. A leaf that calls geo.inFence() and also reads m is REJECTED at compile — not narrowed, rejected — because a location event carries no measurements and a measurement event carries no position, so no event could ever satisfy both and the rule would never fire. For "alarm when a truck in the restricted zone is over 5 degrees", do NOT emit geo.inFence("restricted") && m["tempC"] > 5.0. Emit the containment test alone, or express the second condition as a device ATTRIBUTE, which a location event does carry: geo.inFence("restricted") && "tempC" in attr && attr["tempC"] > 5.0.
