...
| **outbound-connectors** _(opt-in)_ | Delivers REACT's outbound actions to external systems — an HTTP/webhook call and a `publish` to message brokers and cloud queues (MQTT, Kafka, AWS SNS/SQS) — over tenant-scoped, versioned connectors with credentials held in the secret store. Runs in its own process so a slow or misbehaving external system can't touch the detection pipeline. See [Outbound Connectors](./outbound-connectors.md). |
...
| **operator** | A controller-runtime operator that owns the `Instance` custom resource and its lifecycle. Today its reconcile loop only observes the resource; readiness-status aggregation across the rendered Deployments is a planned follow-up that lands on that loop. Config is not reloaded in place: a service reads its configuration once at startup, and a change is adopted by rolling the pods. Workloads themselves are rendered by the Helm chart; tenants are control-plane database records, not reconciled resources. |
...
- **PostgreSQL** stores everything, in two separate databases with one shape. The *relational* store holds entity data — tenants, users, devices, relationships — and the *event* store adds the TimescaleDB extension, keeping time-series events in hypertables with compression and continuous aggregates. `event-management` owns the event store's schema and is the only service that writes events to it or serves them from it, and it talks to nothing else; the one other visitor is `user-management`'s tenant-purge coordinator, which connects to it only to delete a removed tenant's rows. That is why the two can be backed up, sized and restored independently.
...
Integration and provider credentials — an SMTP password, a webhook bearer token, an outbound-connector's broker or cloud credential — and the private half of the key that signs every sign-in token are never stored in plaintext config or a reversible column. They live in an **encrypted secret store**: each value is sealed at rest with a per-secret AES-256-GCM data key wrapped by a key-encryption key (KEK), where the default KEK is a root key on the instance's existing Kubernetes Secret — encryption-at-rest with no additional infrastructure. The store is built around a key-provider seam, so an external key manager can take over wrapping without changing how a consumer stores or resolves a handle; the instance root key is the only provider that ships today. A consumer stores only an opaque **handle**; the value is **write-only over the API** and resolved server-internally at use time, never returned as cleartext. Secret mutations are audited (who, when, which handle — never the value).

## API surface

The platform's data and admin APIs are **GraphQL** (one schema per service), which is introspectable and self-documenting. Internal service-to-service communication is asynchronous over NATS. There is no gRPC, and no REST API over the domain to maintain alongside the schemas. The only non-GraphQL HTTP endpoints are the ones a standard dictates: the JWKS document that token validators fetch, the OAuth 2.1 authorization-server endpoints (`/oauth/authorize`, `/oauth/token`, `/oauth/userinfo`, `/oauth/jwks` and the RFC 8414 metadata document) that `user-management` serves once an issuer URL is configured, and the JSON-RPC-over-HTTP transport spoken by the opt-in [MCP server](./mcp.md).
