...
- **Two provider kinds ship today: `anthropic` and `openai-compatible`.** `anthropic` routes to the Anthropic Claude API, and its endpoint is an optional override of the built-in base URL. `openai-compatible` routes to any endpoint speaking the OpenAI chat-completions API — vLLM, Ollama, DeepSeek, llama.cpp's server, or a gateway in front of them — and because that kind is defined by its address rather than a vendor, its endpoint is **required** and a provider written without one is refused rather than stored unusable. Both kinds count as **external** for the consent gate: the same wire protocol serves an in-cluster vLLM pod and a public API, and the kind alone cannot tell them apart, so a self-hosted `openai-compatible` model still needs the tenant's opt-in. The provider entity is built around a seam for others, and other kinds are refused at write until their implementation lands rather than accepted and left inert.
