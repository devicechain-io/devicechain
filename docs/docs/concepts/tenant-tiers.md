...
:::note Status
**Available today:** the `TenantTier` entity in `user-management`; a config-key registry that defines what a tier packages; tier administration on the instance admin plane; effective-settings resolution through the tier; the AI model-entitlement model (assignments + tier defaults); an **AI packaging** grant matrix; and tier presentation (display order, color, drag-reorder). **Planned:** tiers feed preferential shedding under contention (the tier's stored priority is an operational override) once tenant-priority (ADR-063) lands.
:::
...
## Relationship to ThingsBoard tenant profiles

Tenant tiers reach **parity** with ThingsBoard's tenant profiles — a familiar, expected capability for anyone packaging a multi-tenant IoT platform. It is deliberately **not** marketed as a differentiator; it is table stakes, done cleanly, so that the packaging concept lives in exactly one place instead of being scattered across the services that consume it.
