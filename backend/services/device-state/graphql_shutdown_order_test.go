...
type shutdownProbe struct {
	...
	// inFlight is the service's OWN publish path, run at the same instant. See
...
func (p *shutdownProbe) observeGraphQLStop(natsMgr func() *messaging.NatsManager, subject string) core.LifecycleCallbacks {
	...
	cb.Stopper.Postprocess = func(context.Context) error {
		...
		// The service's own publish path first: it is the thing the ordering exists to
...
func (p *shutdownProbe) assertGraphQLStoppedFirst(t *testing.T) {
	...
	require.NoError(t, p.inFlightErr,
		"a presence demotion still in flight when shutdown began could not write its inbound event: "+
			"the NATS manager was stopped before the HTTP server drained, so the writer "+
			"DemoteAssertedPresence publishes through was already gone")
...
func startShutdownFixture(t *testing.T) (*shutdownProbe, string) {
	...
	// The real writer DemoteAssertedPresence publishes through, on the real connection.
	writer, err := NatsManager.NewWriter(streams.InboundEvents)
	require.NoError(t, err)
	InboundEventsWriter = writer
	probe.inFlight = func() error {
		ctx := core.WithTenant(context.Background(), "acme")
		return InboundEventsWriter.WriteMessages(ctx, messaging.Message{
			Key:   []byte("dev-1"),
			Value: []byte(`{"demotion":"in flight when shutdown began"}`),
		})
	}
...
//
// For this service the hazard is not hypothetical and does not need standing in for:
// DemoteAssertedPresence publishes a synthetic presence event through
// InboundEventsWriter on the caller's own goroutine, and that writer is a writer on this
